...
namespace rs2
{
    ...
    void stream_model::show_stream_header(ImFont* font, const rect &stream_rect, viewer_model& viewer)
    {
        ...
        if (show_metadata)
            stream_model::start_show_metadata(timestamp, timestamp_domain, frame_number, profile, original_size, stream_rect);
    ...
    void stream_model::draw_text_background( const rect stream_rect, const float num_of_lines, const float max_text_width )
    {
        ImGui::GetWindowDrawList()->AddRectFilled(
            { stream_rect.x - 5, stream_rect.y },
            { stream_rect.x + max_text_width * 2,
              stream_rect.y + ( num_of_lines * ( ImGui::GetTextLineHeight() + 3.5f ) ) },
            ImColor( alpha( sensor_bg, 0.2f ) ) );
    }

    void stream_model::start_show_metadata( const long long timestamp,
                                            const rs2_timestamp_domain timestamp_domain,
                                            const unsigned long long frame_number,
                                            ...
                                            const rect & stream_rect )
    {
        ...
        ImGui::SetCursorScreenPos( { stream_rect.x + 10, stream_rect.y } );

        std::string profile_id = rsutils::string::from() << "##Metadata-" << profile.unique_id();
        ImGui::BeginChild( profile_id.c_str(), ImVec2( stream_rect.w - 8, stream_rect.h ) );
        draw_text_background( stream_rect, stream_details.size(), max_text_width );
        ...
        for( auto && at : stream_details )
        {
            ...
            else
            {
                std::string text = "";
                if( at.name != "" )
                    text = rsutils::string::from() << at.name << ":";
                ...
                ImGui::SetCursorPosX( 20 + max_text_width );
